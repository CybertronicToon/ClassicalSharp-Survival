...
namespace ClassicalSharp.GraphicsAPI {
	...
	public abstract partial class IGraphicsApi {
		...
		public int CreateCrackTexture(FastBitmap bmp, bool managedPool, bool mipmaps) {
			if (!Utils.IsPowerOf2(bmp.Width) || !Utils.IsPowerOf2(bmp.Height)) {
				throw new ArgumentOutOfRangeException("Bitmap must have power of two dimensions");
			}
			if (LostContext) throw new InvalidOperationException("Cannot create texture when context lost");
			...
			if (!bmp.IsLocked) bmp.LockBits();		
			int texId = CreateCrackTexture(bmp.Width, bmp.Height, bmp.Scan0, managedPool, mipmaps);
			bmp.UnlockBits();
			return texId;
		}
		...
		protected virtual int CreateCrackTexture(int width, int height, IntPtr scan0, bool managedPool, bool mipmaps) {
			return CreateTexture(width, height, scan0, managedPool, mipmaps);
		}
		...
		public virtual void DrawLine(Vector3 pos1, Vector3 pos2) { }
