...
namespace ClassicalSharp.Renderers {
	...
	public sealed class ChunkUpdater : IDisposable {
		...
		public void Refresh() {
			...
			renderer.liquidPartsCount = new int[TerrainAtlas1D.TexIds.Length];
		...
		internal void ResetUsedFlags() {
			...
			for (int i = 0; i < count; i++) {
				...
				renderer.pendingLiquid[i] = true;
				renderer.usedLiquid[i] = false;
