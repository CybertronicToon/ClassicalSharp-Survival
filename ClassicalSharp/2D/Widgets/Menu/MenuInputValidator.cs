...
namespace ClassicalSharp.Gui.Widgets {
	...
	public class SeedValidator : MenuInputValidator {
		
		long min, max;
		public SeedValidator(long min, long max) {
			this.min = min;
			this.max = max;
			SetRange();
		}
		...
		public SeedValidator() {
			this.min = Int64.MinValue;
			this.max = Int64.MaxValue;
			SetRange();
		}
		...
		public override bool IsValidValue(string s) {
			long value;
			if (!Int64.TryParse(s, out value)) return false;
			return min <= value && value <= max;
		}
	}
