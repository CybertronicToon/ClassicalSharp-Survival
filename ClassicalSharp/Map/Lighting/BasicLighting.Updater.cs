...
namespace ClassicalSharp.Map {
	...
	public sealed partial class BasicLighting : IWorldLighting {
		...
		public override void OnBlockChanged(int x, int y, int z, BlockID oldBlock, BlockID newBlock) {
			...
			byte newSunLight = 15;
			/*if (y < height - 1) {
				if (game.World.GetBlock(x, y + 1, z) == Block.Air) {
			...
			if (BlockInfo.BlocksLight[newBlock] || newBlock == Block.Leaves) {
				...
				if (x > 0)
					SunLightQueue.Enqueue((y * length + z) * width + (x - 1));
				if (y < height - 2)
					SunLightQueue.Enqueue((y * length + z) * width + (y + 1));
				if (y > 0)
					SunLightQueue.Enqueue((y * length + z) * width + (y - 1));
				if (z < game.World.MaxZ)
					SunLightQueue.Enqueue((y * length + z) * width + (z + 1));
				if (z > 0)
					SunLightQueue.Enqueue((y * length + z) * width + (z - 1));*/
			/*} else if (game.World.blocks2[Block] == Block.Water) {
				if (!BlockInfo.BlocksLight[oldBlock]) {
					SetLightFromSun(((y * length + z) * width + x), 0);
					SunLightRemovalQueue.Enqueue(new LightObj(((y * length + z) * width + x), 0);
				}*/
